...
class NewClientController extends SessionController
{   
    private $user;
    ...
    {
        $this->view->render('NewClient/index', [
    ...
    function CreateUser()
    {
        if($this->ExistPOST(['username','password','confirmPassword','phone','identify','address','first_name','second_name','first_lastname','second_lastname']))
        {
                ...
                $phone == ''    || empty($phone))
            {
                ...
                $this->Redirect('newclient',['error' => ErrorMessage::ERROR]);
            }
            ...
            {
                $this->Redirect('newclient',['error' => ErrorMessage::SIGNUP_NEWUSER_EXIST]);
            }
            else
            {
                if($user->Save())
